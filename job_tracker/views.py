...
def job_tracker(request):
    ...
    **Template**
    ...
    jobs = CompletedJob.objects.filter(
        user=user, completed_on__in=working_days_raw).values(
            'completed_on').annotate(total_credits=Sum('job_type__credits'))
    credits_by_day = {d: 0 for d in working_days_raw}
    for entry in jobs:
        if entry['completed_on'] in credits_by_day:
            credits_by_day[entry['completed_on']] = float(
                entry["total_credits"])
    ...
    absences_by_day = {
        a.date: float(a.duration) for a in absences
        if a.date in working_days_raw}
    ...
    if request.method == "POST":
        ...
        if job_form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, 'New job submitted')
    ...
    **Context**
    ...
    if request.method == "POST":
        ...
        if absence_form.is_valid():
            ...
            form.save()
            messages.add_message(
                request, messages.SUCCESS, 'New absence submitted')
        else:
            messages.add_message(
                request, messages.ERROR, 'Error sumbitting absence')
    ...
    **Context**
    ...
    if request.method == "POST":
        ...
        if absence_form.is_valid():
            ...
            messages.add_message(
                request, messages.ERROR, 'Error updating absence')
    return redirect('absences')
    ...
    **Context**
    ...
    if request.method == "POST":
        ...
        if profile_form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, 'Error updating settings')
